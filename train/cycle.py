...
from datasets.cycle import CycleDataset
    ...
    for i, (X, Y) in enumerate(progress):
        ...
        if i % 200 == 0:
            if co:
                ...
                X = unnorm(X)
                Y = unnorm(Y)
            save_image(X, f"results/{epoch}_{i//200}_X.png")
            save_image(X_fake, f"results/{epoch}_{i//200}_X_f.png")
            save_image(Y, f"results/{epoch}_{i//200}_Y.png")
            save_image(Y_fake, f"results/{epoch}_{i//200}_Y_f.png")
...
def main():
    ...
    dataset = CycleDataset(
        root_X="/media/x/horse2zebra/trainA/",
        root_Y="/media/x/horse2zebra/trainB/",
