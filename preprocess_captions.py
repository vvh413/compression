from datasets.image_captioning import coco_raw, coco_preprocessed
...
coco_train = coco.CocoCaptions(coco_raw, "datasets/image_captioning/annotations/captions_train2017.json", transform=config.transform_train)
dataloader = torch.utils.data.DataLoader(dataset=coco_train, batch_size=32, shuffle=False, num_workers=4)
...
for img_batch, capt_batch in tqdm(dataloader):
    ...
    img_batch_compressed = F.max_pool2d(img_batch_compressed, kernel_size=2) #.view(32, -1)
...
with open(os.path.join(coco_preprocessed, "co_captions_tokenized_8m.json"), 'w') as f_cap:
